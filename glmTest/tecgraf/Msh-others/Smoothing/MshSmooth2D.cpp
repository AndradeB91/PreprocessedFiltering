//
// MshSmooth2D implementation file
//
// Description -
...
//
// Revision -
//   $Revision: 1.24 $  $Date: 2004/06/01 15:46:45 $  $Author: wash $
//
...
// %(MshSmooth2D::MshSmooth2D-constructor-|)
/* ++ ----------------------------------------------------------
**
...
MshSmooth2D::MshSmooth2D()
{
    NodeTable = new Dict<int,IntNode> ;
    ElemTable = new Dict<int,MshElement2D> ;
    ZCoords   = new Dict<int,double> ;
    ...
    Dict<int,MshElement2D> *elem_tbl)
{
    ...
    Dict<int,MshElement2D>::DictIterator iter(ElemTable) ;
    MshElement2D& elem = iter.Entry() ;
...
**
**      void AddNode(const MshNode &inode)
**
...
**
**      Exceptions:
**          MshCDuplNode - duplicate node id's
**
** -- */

void MshSmooth2D::AddNode(
    int id,
    const Vec2D& coord,
    double z_coord)
{
    if (NodeTable->HasKey(id)) throw DuplicateNodeError() ;

    IntNode tmp ;
    tmp.id = id ;
    tmp.coord = coord ;
    tmp.motion = MSH_FLOATING ;
    tmp.corner = false ;
    NodeTable->Store(id,tmp) ;
    ZCoords->Store(id,z_coord) ;
    FindBound = true ;
}
...
**
**      Exceptions:
**          MshCDuplElem - duplicate element id's
**          MshCInvalidElem - invalid element
**
** -- */

void MshSmooth2D::AddElem(
    int elem_id,
    int mat_id,
    int num_nodes,
    const int nodes[])
{
    if (ElemTable->HasKey(elem_id)) throw DuplicateElemError() ;
    ...
    if (FirstElem) {
        ...
        if ((num_nodes == 3) || (num_nodes == 4)) {
            if (Order != LINEAR) throw ElementOrderError() ;
        } else if ((num_nodes == 6) || (num_nodes == 8)) {
            if (Order != QUADRATIC) throw ElementOrderError() ; ;
        } else {
    ...
    ElemTable->Store(elem_id,MshElement2D(elem_id,mat_id,num_nodes,nodes)) ;
    FindBound = true ;
}
...
void MshSmooth2D::SmoothNodesLaplace()
{
    if (FindBound) {
        FindBoundaryNodes() ;
        FindBound = false ;
    ...
    MshTopo2D *msh_topo = BuildMeshTopo() ;
...
void MshSmooth2D::SmoothNodesWinslow()
{
    if (FindBound) {
        FindBoundaryNodes() ;
        FindBound = false ;
    ...
    DoSmoothNodesLaplace(msh_topo,1,min_length*It_tol) ;
...
void MshSmooth2D::SmoothNodesConsLaplace()
{
    if (FindBound) {
        FindBoundaryNodes() ;
        FindBound = false ;
...
Vec2D MshSmooth2D::ConsLaplaceUpdate(int num_elem,
                               List<int> *num_adj_nodes,
                               List<Vec2D> *adj_coords)
{
    int i, j, k ;
    ...
    for (i=0 ; i<num_elem ; ++i) {
        if (num_adj_nodes->At(i) == 4) {
            ...
            lines.Append(adj_coords->At(cur)) ;
            lines.Append(Vec2D(adj_coords->At(cur+1)-
                                          adj_coords->At(cur))) ;

            lines.Append(adj_coords->At(cur+3)) ;
            lines.Append(Vec2D(adj_coords->At(cur)-
                                          adj_coords->At(cur+3))) ;
...
if (fd != 0) {
  fprintf(fd,"    pts[] = Vec2D(%23.15e,%23.15e);\n",lines[i][0],lines[i][1]) ;
  fprintf(fd,"    pts[] = Vec2D(%23.15e,%23.15e);\n",lines[i+1][0],lines[i+1][1]) ;
  fprintf(fd,"---\n") ;
  List<Vec2D> *tmp = feasable.GetVertices() ;
  for (int ii=0 ; ii<tmp->Len() ; ++ii) {
    fprintf(fd,"v %g %g 0\n",tmp->At(ii)[0],tmp->At(ii)[1]) ;
  }
}
    ...
    if (poly->Len() != 0) {
        ...
        delete poly ;
        return (Vec2D(center.x() + sum_prod_x/sum_area,
...
void MshSmooth2D::DoSmoothNodesConsLaplace(MshTopo2D *msh_topo,
                                               int num_iters)
{
    int j ;
    ...
    Dict<int,IntNode>::DictIterator niter(NodeTable) ;
    for (j=0 ; j<num_iters ; ++j) {
        for (niter.First() ; niter.More() ; ++niter){

            IntNode& node = niter.Entry() ;
            if ((node.motion == MSH_FIXED) || (!node.corner)) continue ;
            ...
            for (iter.First() ; iter.More() ; ++iter) {
                if (iter.CcwElem() == NO_ELEM) continue ;
                int nnodes,nodes[4] ;
                msh_topo->GetElemNodes(iter.CcwElem(),
                                       node.id,&nnodes,nodes) ;
                num_adj_nodes.Append(nnodes) ;
                if (nnodes == 4) {
                    for (j=0 ; j<4 ; ++j) {
                        IntNode *adj =
                           NodeTable->Get(nodes[(j+2)%4]) ;
                        adj_coords.Append(adj->coord) ;
                    }
                } else {
                    for (j=0 ; j<3 ; ++j) {
                        IntNode *adj =
                          NodeTable->Get(nodes[(j+1)%3]) ;
                        adj_coords.Append(adj->coord) ;
                    }
                }
                ++num_adj ;
...
static Vec2D WinslowEval(int num_elem,Vec2D node_coord,
                               int num_adj,Vec2D *adj_coord,
                               double *ct,double *st)
{
    int i ;
    Vec2D D_psi(0,0) ;
    ...
    if (num_elem == 4) {
        for (i=0 ; i<4 ; ++i) {
            Vec2D delta = adj_coord[i] - node_coord ;
            D_psi += delta * ct[i] ;
            D_eta += delta * st[i] ;
    ...
    Vec2D D_x = (D_eta*D_eta)*D_psi_psi -
...
static Vec2D WinslowUpdate(int num_elem,Vec2D node_coord,
                                 int num_adj,Vec2D *adj_coord,
                                 double *ct,double *st)
{
    // Determine a tolerance

    Vec2D mean = 0.5*(adj_coord[0]+adj_coord[1]) ;
    double tol = 0.01 * (node_coord - mean).Magnitude() ;

    Vec2D coord,lastf,lastc ;
    ...
    int iter = 0 ;
    while (!done) {
        Vec2D eval = WinslowEval(num_elem,coord,num_adj,
                                       adj_coord,ct,st) ;
        if (iter >= MAX_WITER) {
            done = true ;
        } else {
            Vec2D slope = (lastf - eval) / (lastc - coord) ;
            lastf = eval ;
            lastc = coord ;
            coord -= eval / slope ;
...
void MshSmooth2D::DoSmoothNodesWinslow(MshTopo2D *msh_topo,
                                           int max_it,double adapt_tol)
{
    ...
    fprintf(stderr,"%d \n",Num_it) ;
        ...
        for (niter.First() ; niter.More() ; ++niter){
            ...
            // loop through the adjacent elements, store their coordinates
            ...
            // compute the initial shape measures

            int cur = 0 ;
            for (i=0 ; i<num_adj ; ++i) {
                if (num_adj_nodes[i] == 4) {
                    sm = QuadMetric(adj_coords[cur],
                                    adj_coords[cur+1],
                                    adj_coords[cur+2],
                                    adj_coords[cur+3]) ;
                    cur += 4 ;
                } else {
                    sm = TriMetric(adj_coords[cur],
                                   adj_coords[cur+1],
                                   adj_coords[cur+2]) ;
                    cur += 3 ;
                }
            ...
            if (num == 2) {
                ...
                if (iter.CcwElem() == NO_ELEM) continue ;
                    ...
                    msh_topo->GetElemNodes(iter.CcwElem(),
                                       node.id,&nnodes,nodes) ;
                    if (nnodes == 4) {
                        IntNode *adj = NodeTable->Get(nodes[2]) ;
                        adj_coord[num_adj] = adj->coord ;
            ...
            // check to make sure that this does not make the
            // mesh fold
...
// %(MshSmooth2D::DoSmoothNodesLaplace-void-|)
/* ++ ----------------------------------------------------------
**
...
void MshSmooth2D::DoSmoothNodesLaplace(MshTopo2D *msh_topo,
                                           int max_it,double adapt_tol,
                                           bool skip_checks)
{
    int j ;
...
// %(MshSmooth2D::LaplaceUpdate-Vec2D-|-int-|-List-|<int>*-List-|<Vec2D>*)
/* ++ ----------------------------------------------------------
**
...
Vec2D MshSmooth2D::LaplaceUpdate(int num_elem,
                               List<int> *num_adj_nodes,
                               List<Vec2D> *adj_coords,
                               bool skip_checks)
{
...
//bool start_neg = false ;
    ...
    Vec2D delta = coord/double(num_elem) - adj_coords->At(2) ;
    ...
    for (int step=0 ; step<10 ; ++step) {
        ...
        for (i=0 ; i<num_elem ; ++i) {
            ...
            if (sm <= 0.0) have_neg = true ;
            if ((shape[i] > 0.0) && (sm <= 0.0)) goes_neg = true ; 
            if ((shape[i] < 0.0) && (sm < shape[i])) goes_neg = true ; 
...
double MshSmooth2D::FindMinCharEdgeLength(MshTopo2D *msh_topo)
{
    ...
    for (niter.First() ; niter.More() ; ++niter){
        ...
        for (iter.First() ; iter.More() ; ++iter) {
            ...
            IntNode* anode = NodeTable->Get(iter.AdjVtx()) ;
...
// %(MshSmooth2D::FindBoundaryNodes-void-|)
/* ++ ----------------------------------------------------------
**
...
void MshSmooth2D::FindBoundaryNodes()
{
    Dict<EdgeKey,int> edges ;

    Dict<int,MshElement2D>::DictIterator eiter(ElemTable) ;
    for (eiter.First() ; eiter.More() ; ++eiter) {
        ...
        for (int i=0 ; i<nn ; ++i) {
            ...
            if (elem.nodes[i] < elem.nodes[j])
                key = EdgeKey(elem.nodes[i],elem.nodes[j]) ;
            else
                key = EdgeKey(elem.nodes[j],elem.nodes[i]) ;

            IntNode *nd = NodeTable->Get(elem.nodes[i]) ;
            nd->corner = true ;
            nd = NodeTable->Get(elem.nodes[j]) ;
            nd->corner = true ;
    ...
    Dict<EdgeKey,int>::DictIterator iter(&edges) ;

    for (iter.First() ; iter.More() ; ++iter){
        IntNode *nd = NodeTable->Get(iter.Key().id_0) ;
        nd->motion = MSH_FIXED ;
        nd = NodeTable->Get(iter.Key().id_1) ;
        nd->motion = MSH_FIXED ;
    }
}
...
// %(MshSmooth2D::BuildMeshTopo-MshTopo2D-|*)
/* ++ ----------------------------------------------------------
**
...
MshTopo2D *MshSmooth2D::BuildMeshTopo()
{
    MshTopo2D *msh_topo = new MshTopo2D() ;
...
// %(MshSmooth2D::MoveSideNodes-void-|)
/* ++ ----------------------------------------------------------
**
...
void MshSmooth2D::MoveSideNodes()
{
    int j ;
    Dict<int,MshElement2D>::DictIterator eiter(ElemTable) ;
    for (eiter.First() ; eiter.More() ; ++eiter) {
        ...
        for (j=0 ; j<nn ; ++j) {
            ...
            IntNode *pside = NodeTable->Get(side) ;

            if (pside->motion == MSH_FLOATING) {
                IntNode *pprev = NodeTable->Get(prev) ;
                IntNode *pnext = NodeTable->Get(next) ;
...
// %(MshSmooth2D::NumNodes-int-|^const)
/* ++ ----------------------------------------------------------
**
...
int MshSmooth2D::NumNodes() const
{
    return(NodeTable->Len()) ;
}




// %(MshSmooth2D::NumElements-int-|^const)
/* ++ ----------------------------------------------------------
**
...
int MshSmooth2D::NumElements() const
{
    return(ElemTable->Len()) ;
}



// // %(MshSmooth2D::GetNodes-MshNode-|*^const)
// /* ++ ----------------------------------------------------------
// **
// **    GetNodes - return a list of all mesh nodes 
// **
// **      MshNode *GetNodes() const
// **
...
// ** -- */
// 
// MshNode *MshSmooth2D::GetNodes() const
// {
//     int i ;
//     MshNode   *local_entries ;
// 
//     Dict<int,IntNode>::DictIterator iter(NodeTable) ;
//     local_entries = new MshNode[NodeTable->Len()] ;
// 
...
// // %(MshSmooth2D::GetElements-MshElement2D-|*^const)
// /* ++ ----------------------------------------------------------
// **
...
// ** -- */
// 
// MshElement2D *MshSmooth2D::GetElements() const
// {
//     Dict<int,MshElement2D>::DictIterator iter(ElemTable) ;
//     MshElement2D *local_elems = 
//         new MshElement2D[ElemTable->Len()] ;
// 
...
void MshSmooth2D::DisplayMesh(char *label,FILE *infd)
{
//    bool do_labels = false ;
    bool do_labels = true ;
    FILE *fd = (infd == 0) ? stdout : infd ;
    ...
    for (eiter.First() ; eiter.More() ; ++eiter) {
        ...
        for (int j=0 ; j<elem.num_nodes ; ++j) {
            IntNode *node = NodeTable->Get(elem.nodes[j]) ;
            x[j] = node->coord[0] ;
            y[j] = node->coord[1] ;
            id[j] = node->id ;
        }
...
// %(MshSmooth2D::Angle-double-|^const-Vec2D-|-Vec2D-|-Vec2D-|)
/* ++ ----------------------------------------------------------
**
...
double MshSmooth2D::Angle(Vec2D b,Vec2D i,
                              Vec2D j) const
{
    Vec2D bi = i - b ;
    Vec2D bj = j - b ;
...
// %(MshSmooth2D::CrossProd-double-|^const-Vec2D-|-Vec2D-|-Vec2D-|)
/* ++ ----------------------------------------------------------
**
...
double MshSmooth2D::CrossProd(Vec2D b,Vec2D i,
                                  Vec2D j) const
{
    double cross ;
...
// %(MshSmooth2D::TriMetric-double-|^const-Vec2D-|-Vec2D-|-Vec2D-|)
/* ++ ----------------------------------------------------------
**
...
// %(MshSmooth2D::QuadMetric-double-|^const-Vec2D-|-Vec2D-|-Vec2D-|-Vec2D-|)
/* ++ ----------------------------------------------------------
**
