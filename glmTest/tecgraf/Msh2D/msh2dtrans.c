...
)
{
  ...
  *nno=index;
  ...
  for(i=0;i<*nno;i++)
  {
    points[i*2+0]=points[list2[i]*2+0];
    points[i*2+1]=points[list2[i]*2+1];
  }
  /* Sort conn */
  index=0;
  for(i=0;i<nel;i++)
 ...
 *nno=m*n; /* number of nodes */
 ...
 /* alocate memory */
 *pt = (double *)calloc ((*nno)*2, sizeof(double));
 if (*pt == NULL)
  return 0;
 *conn = (int *)calloc ((*nel)*(elem_type+1), sizeof(int));
 ...
 Msh2DTransfin(bry,m,n,elem_type,diagtype,*pt,*conn);
...
***************************************************************************
*/
int Msh2DTryBilinear (double *bry, int np, int elem_type, int diagtype, 
                      int *nno, int *nel, double **pt, int **conn)
{
  int m, n;

  if (!Msh2DAutomaticBilinearCornes(np, bry, &m, &n))
    return 0;

  if (!Msh2DBilinear (bry, m, n, elem_type, diagtype, nno, nel, pt, conn))
    return 0;
...
**      conn   -> elements connectivity
*/
int Msh2DLoft (double *bry, int m, int n, int dir, double weight,
 ...
 *conn = (int *)calloc ((*nel)*(elem_type+1), sizeof(int));
 ...
 Msh2DLofting(bry,m,n,dir,weight,elem_type,diagtype,*pt,*conn);
...
**	conn   -> elements connectivity
*/
int Msh2DTrilinear (double *bry, int m, int elem_type,  
                  int *nno, int *nel, double **pt, int **conn)
{
  int     i;
 ...
 *conn = (int *)calloc ((*nel)*(elem_type+1), sizeof(int));
 ...
 Msh2DTrimap(bry,m,elem_type,*pt,*conn);
...
***************************************************************************
*/
int Msh2DTryTrilinear (double *bry, int np, int elem_type, 
                      int *nno, int *nel, double **pt, int **conn)
{
  int m;

  if (!Msh2DAutomaticTrilinearCornes(np, bry, &m))
    return 0;

  if (!Msh2DTrilinear(bry, m, elem_type, nno, nel, pt, conn))
    return 0;
...
int Msh2DCollBilinear (double *bry, int m, int n, int elem_type, int diagtype,  
                        int *nno, int *nel, double **pt, int **conn)
{
 ...
 *conn = (int *)calloc ((*nel)*(elem_type+1), sizeof(int));
 if (*conn == NULL)
  return 0;
   ...
   Msh2DTrsfncoll(bry,m,n,elem_type,diagtype,*pt,*conn); 
  ...
  *nno=*nno-blank;
  for(i=1;i<*nno;i++)
  {
	  (*pt)[i*2+0]=(*pt)[(i+blank)*2+0];
	  (*pt)[i*2+1]=(*pt)[(i+blank)*2+1];
  }
  /* Sort conn */
  index=0;
  for(i=0;i<*nel;i++)
...
int Msh2DCollLoft (double *bry, int m, int n, double weight,
                   int elem_type, int diagtype,
                   int *nno, int *nel, double **pt, int **conn)
{
  int     nsegs = 0; /* number of segments int the opposite to the collapsed node */
 ...
 *conn = (int *)calloc ((*nel)*(elem_type+1), sizeof(int));
 if (*conn == NULL)
  ...
  Msh2DLoftcoll(bry,m,n,weight,elem_type,diagtype,*pt,*conn); 
